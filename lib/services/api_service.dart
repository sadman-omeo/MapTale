  ...
  static Future<List<Landmarks>> getLandmarks() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decoded is List) {
          landmarks = decoded.map((item) => Landmarks.fromJson(item)).toList();
        } else if (decoded is Map && decoded['data'] is List) {
          landmarks = (decoded['data'] as List)
              .map((item) => Landmarks.fromJson(item))
              .toList();
        } else {
          throw Exception('Unexpected API response format');
        }

        await OfflineService.saveLandmarksCache(landmarks);
        return landmarks;
      } else {
        throw Exception('Failed to load Landmarks!');
      }
    } catch (e) {
      final cached = await OfflineService.loadLandmarksCache();
